...
class Plansza(object):
    ...
    def usun_pionek_z_planszy(self, pionek):
        root_logger.debug("Usuwam pionek na pozycji " + pozycja_to_str(pionek.pozycja))
    ...
    def generuj_mozliwe_ruchy(self, pozycja_startowa, czy_poprzedni_ruch_byl_biciem, drzewo_obowiazkowych_bic, lista_mozliwych_ruchow):
        ...
        for ruch in pionek.get_wektory_ruchu():
            root_logger.debug("Przetwarzam sciezke od " + pozycja_to_str(pozycja_startowa) + " i ruch o wektor " + str(ruch))
            ...
            if self.slownikPozycji.get(pozycja_do) is not None:
                ...
                root_logger.debug("Mamy bicie, a nastepna mozliwa pozycja to " + pozycja_to_str(pozycja_po_biciu) )
                ...
                if czy_poprzedni_ruch_byl_biciem is not True:
                   root_logger.debug("      Pole jest wolne, mozloiwy ruch, ale bez bicia")
    ...
    def mozliwe_ruchy(self, gracz = None):
        ...
        for pionek in gracz.pionki:
            ...
            if drzewo_obowiazkowych_bic is not None and drzewo_obowiazkowych_bic.puste() == False:
    ...
    def rysujPlansze(self):
        ...
        #Dol planszy
        linie.append(chr(self.wysokosc+64) + '  ' + ' │ ' + ' │ '.join(self.stanPlanszy[-1]) + ' │') #+ str(num + 1) +
        linie.append('    ╰' + ('───┴' * (self.szerokosc-1)) + '───╯')
